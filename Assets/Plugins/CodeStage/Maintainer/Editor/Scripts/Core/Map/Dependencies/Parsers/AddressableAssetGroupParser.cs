	...
	public class AddressableAssetGroupParser : IDependenciesParser
	{
		...
		private static List<string> ExtractReferencedAssets(Object assetGroup)
		{
			...
			if (serializedEntries == null)
			{
				...
				if (serializedEntries == null)
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Can't reach serialize entries in AddressableAssetGroup!"));
					return null;
				}
			}
			...
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Can't find serialize entries array in AddressableAssetGroup!"));
				return null;
			}
			...
			for (var i = 0; i < count; i++)
			{
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Can't reach Serialize entry GUID of AddressableAssetGroup!"));
					return null;
				}
