		...
		private static bool AssetInIgnores(AssetInfo assetInfo, List<string> ignoredScenes)
		{
			...
			if (assetInfo.Type == CSReflectionTools.spriteAtlasType)
			{
				...
				if (atlas != null)
				{
					...
					{
						Debug.LogError(Maintainer.ConstructErrorForSupport("Can't parse UnityEngine.U2D.SpriteAtlas!", ModuleName));
					}
				}
				else
				{
					Debug.LogWarning(Maintainer.ConstructErrorForSupport("Couldn't load SpriteAtlas: " + assetInfo.Path, ModuleName));
				}
			}
