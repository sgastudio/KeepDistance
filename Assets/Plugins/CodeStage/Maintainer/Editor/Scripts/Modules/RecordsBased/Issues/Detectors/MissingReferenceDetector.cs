...
namespace CodeStage.Maintainer.Issues.Detectors
{
	...
	internal class MissingReferenceDetector : IssueDetectorBase
	{
		...
		public void TryDetectUnityEventIssues(RecordLocation location, string assetPath, GameObject target, Type componentType, string componentName, int orderIndex, SerializedProperty property)
		{
			...
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_PersistentCalls in serialized UnityEvent!", IssuesFinder.ModuleName));
				return;
			}
			...
			for (var i = 0; i < callsCount; i++)
			{
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_Target in serialized UnityEvent's call!", IssuesFinder.ModuleName));
					return;
				}
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_MethodName in serialized UnityEvent's call!", IssuesFinder.ModuleName));
					return;
				}
				...
				if (arguments == null)
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_Arguments in serialized UnityEvent's call!", IssuesFinder.ModuleName));
					return;
				}
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_ObjectArgumentAssemblyTypeName in m_Arguments!", IssuesFinder.ModuleName));
					return;
				}
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find m_Mode in serialized UnityEvent's call!", IssuesFinder.ModuleName));
					return;
				}
				...
				if (dummyEvent == null)
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't get something from GetDummyEvent!", IssuesFinder.ModuleName));
					return;
				}
		...
		public static bool IsPropertyHasMissingReference(SerializedProperty currentProperty)
		{
			...
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Property seems to be missing reference but m_FileID could not be found!",
