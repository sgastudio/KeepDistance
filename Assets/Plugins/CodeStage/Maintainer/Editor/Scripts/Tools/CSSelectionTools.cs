	...
	internal static class CSSelectionTools
	{
		...
		public static void RevealAndSelectReferencingEntry(string assetPath, ReferencingEntryData referencingEntry)
		{
			    ...
			    referencingEntry.location == Location.SceneNavigationSettings))
			{
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Can't open scene " + assetPath));
			...
			switch (referencingEntry.location)
			{
				...
				case Location.SceneLightingSettings:
					...
					{
						Debug.LogError(Maintainer.ConstructErrorForSupport("Can't open Lighting settings!"));
						MaintainerWindow.ShowNotification("Can't show it properly");
					}
				...
				case Location.SceneNavigationSettings:
					...
					{
						Debug.LogError(Maintainer.ConstructErrorForSupport("Can't open Navigation settings!"));
						MaintainerWindow.ShowNotification("Can't show it properly");
					}
		...
		private static bool RevealAndSelectGameObjectInScene(string path, string transformPath, long objectId, long componentId)
		{
			...
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Target scene is not valid or not found! Scene path: " + path + ", looked for ObjectID " + objectId + "!"));
				return false;
			}
			...
			if (target == null)
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't find target Game Object " + transformPath + " at " + path + " with ObjectID " + objectId + "!"));
				return false;
			}
		...
		private static bool RevealAndSelectGameObjectInPrefab(string path, string transformPath, long objectId, long componentId)
		{
			...
			else
			{
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't open prefab at " + path + "!"));
					return false;
				}
				...
				if (prefabStage == null)
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Couldn't get prefab stage for prefab at " + path + "!"));
					return false;
				}
